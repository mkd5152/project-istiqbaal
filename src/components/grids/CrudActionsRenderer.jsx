...
function Icon({ type }) {
  if (type === 'edit') {
    return (
      <svg viewBox="0 0 24 24" style={icon} aria-hidden>
        <path fill="currentColor" d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1.003 1.003 0 0 0 0-1.42L18.37 3.29a1.003 1.003 0 0 0-1.42 0l-1.83 1.83 3.75 3.75 1.84-1.83z"/>
      </svg>
    );
  }
  if (type === 'save') {
  ...
  if (type === 'cancel') {
    return (
      <svg viewBox="0 0 24 24" style={icon} aria-hidden>
        <path fill="currentColor" d="M18.3 5.71L12 12l6.3 6.29-1.41 1.42L10.59 13.4 4.3 19.71 2.89 18.3 9.18 12 2.89 5.71 4.3 4.29 10.59 10.6l6.3-6.31z"/>
      </svg>
    );
  }
  if (type === 'delete') {
...
export default function CrudActionsRenderer(props) {
  ...
  return (
    <Row>
      ...
      <button
        onClick={() => context?.onDeleteExisting?.(data)}
        title="Delete"
        style={theme.danger}
        onMouseEnter={(e) => Object.assign(e.currentTarget.style, hover)}
        onMouseLeave={(e) => Object.assign(e.currentTarget.style, { transform: '', boxShadow: '' })}
